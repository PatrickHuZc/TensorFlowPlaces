...
step_display = 100
step_save = 2000
path_save = 'alexnet_minus_fc_double_filtered'
start_from = ''

def alexnet(x, filter_kernel):
    weights = {
        ...
        'wt_o': tf.Variable(tf.random_normal([7*7*256, 100], stddev=np.sqrt(2./(7*7*256)))),
        'wb_o': tf.Variable(tf.random_normal([7*7*256, 100], stddev=np.sqrt(2./(7*7*256))))
    ...
    # Apply filter
    x_bot = tf.nn.depthwise_conv2d(x, filter_kernel, strides=[1, 1, 1, 1], padding='SAME')
    for i in range(blur_count - 1):
        x_bot = tf.nn.depthwise_conv2d(x_bot, filter_kernel, strides=[1, 1, 1, 1], padding='SAME')
    ...
    pool5_t_temp = tf.reshape(pool5_t, [-1, weights['wt_o'].get_shape().as_list()[0]])
    pool5_b_temp = tf.reshape(pool5_b, [-1, weights['wb_o'].get_shape().as_list()[0]])

    ###############################################################################################
    #### Singleton branch
    
    out = tf.add(tf.matmul(pool5_t_temp, weights['wt_o']) + tf.matmul(pool5_b_temp, weights['wb_o']), biases['bo'])
